...
async def seed():
    async with SessionLocal() as db:
        user = (await db.execute(select(User).where(User.email == "demo@local"))).scalar_one_or_none()
        if user is None:
            user = User(email="demo@local", name="Demo", password_hash=hash_password("demo1234"))
            db.add(user)
            await db.flush()
        ...
        await db.commit()
        print("Seed concluído: demo@local / demo1234")
